function [net,stats] = cnn_train_dag_modified(net, imdb, getBatch, varargin)
...
for i=1:numel(net.params)
    ori.learningRate(i)=net.params(i).learningRate;
    ori.weightDecay(i)=net.params(i).weightDecay;
end
...
for epoch=start+1:opts.numEpochs
  if epoch >1
      firstHalfOff=opts.firstHalfOff(min(epoch, numel(opts.firstHalfOff))) ;
      if ~exist('former_firstHalfOff')
          former_firstHalfOff=opts.firstHalfOff(epoch-1);
      end
      if firstHalfOff~=former_firstHalfOff
          if isa(net, 'Composite'), net = net{1} ; end
          if firstHalfOff
              for param_i=1:numel(net.params)
                  if ~isempty(strfind(net.params(param_i).name,'T_'))&&...
                          isempty(strfind(net.params(param_i).name,'classifier'))
                     net.params(param_i).learningRate=0;
                     net.params(param_i).weightDecay=0;
                  end
              end
          else
              for param_i=1:numel(net.params)
                  if ~isempty(strfind(net.params(param_i).name,'T_'))&&...
                          isempty(strfind(net.params(param_i).name,'classifier'))
                     net.params(param_i).learningRate=ori.learningRate(param_i);
                     net.params(param_i).weightDecay=ori.weightDecay(param_i);
                  end
              end
          end
      former_firstHalfOff=firstHalfOff;
      end
  else
      former_firstHalfOff=opts.firstHalfOff(min(epoch, numel(opts.firstHalfOff))) ;
  end
  ...
  params.getBatch = getBatch ;
   ...
   if opts.restrictionOff(min(epoch, numel(opts.restrictionOff)))
       if isa(net, 'Composite'), net = net{1} ; end
       for i_der=1:numel(params.derOutputs)
           if strcmp(params.derOutputs{i_der},'restriction_loss')
               params.derOutputs{i_der+1}=0;
           end
       end
   end
...
sel = [find(cellfun(@(x) isa(x,'dagnn.Loss'), {net.layers.block})),...
    find(cellfun(@(x) isa(x,'PairwiseTripletLoss'), {net.layers.block}))];
for i = 1:numel(sel)
